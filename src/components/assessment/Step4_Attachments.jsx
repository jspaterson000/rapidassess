...
export default function Step4_Attachments({ photos, documents, onPhotosUpdate, onDocumentsUpdate, onNext, onBack }) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="text-center">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">General Attachments</h1>
        <p className="text-gray-600">Upload additional photos and supporting documents for this assessment</p>
      </div>
      ...
      {!hasFiles && (
        <div className="text-center py-8 bg-amber-50 rounded-xl border border-amber-200">
          ...
          <h3 className="font-semibold text-amber-800 mb-2">No files uploaded yet</h3>
          <p className="text-amber-700">You can skip this step or add files using the upload areas above</p>
        </div>
      )}
